package TreeOfSegments.Part1.Lesson2.problemC;
...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for (int j = 0; j < m; j++) {
            ...
            if (var == 1) {
                ...
                int x = input.nextInt();
                answer.append(segmentTree.firstAbove(x)).append("\n");
            }
        }
    ...
    static class FindFirstElemMoreOrEqualsThen {
        ...
        public int firstAbove(int v) {
            return firstAbove(v, 0, 0, size);
        }

        private int firstAbove(int v, int x, int lx, int rx) {
            if (tree[x] < v) {
                return -1;
            }
            if (rx - lx == 1) {
            ...
            int res = firstAbove(v, 2 * x + 1, lx, m);
            if (res == -1) {
                res = firstAbove(v, 2 * x + 2, m, rx);
            }
